...
namespace Anarchy
{
	...
	void CreateTitleScene(Scene& scene, const Window& window, Scene& gameScene)
	{
		...
		UIFactory ui = layer.GetUI().GetRoot().GetFactory();
		UIRectangle& background = ui.CreateRectangle(500, 600, Color::White, Transform({ window.Width() / 2.0f, window.Height() / 2.0f, 0 }));
		background.GetFactory().CreateText("Anarchy", titleFont, Color::Black, Transform({ 0, 125, 1 }));

		UITextInput& serverInput = background.GetFactory().CreateTextInput(300, 50, displayFont, Color::Black, Color(128, 128, 128), Transform({ 0, 0, 1 }));
		serverInput.GetFactory().CreateText("Server:", displayFont, Color::Black, Transform({ -150, 30, 1 }), AlignH::Left, AlignV::Bottom);
		serverInput.SetText("localhost:10000");

		UIRectangle& connectButton = background.GetFactory().CreateRectangle(300, 50, Color(50, 200, 50), Transform({ 0, -60, 1 }));
		connectButton.GetFactory().CreateText("Connect", displayFont, Color::Black, Transform({ 0, 0, 1 }));
		...
		background.GetFactory().CreateText("Connecting As: Totomosic", displayFont, Color::Black, Transform({ 0, -100, 1 }));
	}
